...
export const PORTAL_TOKEN_KEY = 'cms_member_token';
...
export async function portalApi<T = unknown>(path: string, options: PortalRequestOptions = {}): Promise<T> {
  ...
  if (response.status === 401) {
    ...
    throw new PortalApiError(401, 'Session expired. Please log in again.');
  }
...
export function portalDownload(path: string, filename: string) {
  const token = getPortalToken();
  if (!token) {
    if (typeof window !== 'undefined' && !window.location.pathname.startsWith('/portal/login')) {
      window.location.href = '/portal/login';
